...
class ComplexFeatureExtractor(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(13, 64)  # 第一个全连接层，将13维特征转换为64维
        self.fc2 = nn.Linear(64, 32)  # 第二个全连接层，将64维特征转换为32维
        self.fc3 = nn.Linear(32, 16)  # 第三个全连接层，将32维特征转换为16维
        self.fc4 = nn.Linear(16, 1)   # 第四个全连接层，将16维特征转换为1维

    def forward(self, x):
        x = torch.relu(self.fc1(x))  # 使用ReLU激活函数
        x = torch.relu(self.fc2(x))  # 使用ReLU激活函数
        x = torch.relu(self.fc3(x))  # 使用ReLU激活函数
        x = self.fc4(x)
        return x
    
class ComplexTargetLinearFeatureExtractor(nn.Module):
    def __init__(self, input_dim, hidden_size, output_dim):
        super(ComplexTargetLinearFeatureExtractor, self).__init__()
        self.fc1 = nn.Linear(input_dim,  hidden_size,)  # 第一个全连接层，将输入特征转换为64维
        # self.fc2 = nn.Linear( hidden_size, hidden_size,)        # 第二个全连接层，将64维特征转换为32维
        self.fc2 = nn.Linear( hidden_size, output_dim)  # 第四个全连接层，将16维特征转换为输出维度

    def forward(self, x):
        x = torch.relu(self.fc1(x))  # 使用ReLU激活函数
        x = torch.relu(self.fc2(x))  # 使用ReLU激活函数
        # x = self.fc3(x)
        return x  # 定义新的类，使其中具体的网络层可以通过形参赋值
