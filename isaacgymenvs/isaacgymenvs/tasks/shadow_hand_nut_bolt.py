...
class ShadowHandNutBolt(VecTask):
    """
    ...
    def __init__(self, cfg, rl_device, sim_device, graphics_device_id, headless, virtual_screen_capture, force_render):
        ...
        self.asset_files_dict = {
            ...
            "nut":"/mjcf/factory/urdf/factory_nut_m4_loose.urdf",
            "bolt":"/mjcf/factory/urdf/factory_bolt_m4_loose.urdf"
        }
        ...
        # create some wrapper tensors for different slices  为不同切片创建一些包装张量
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        #名看起来像是用于从文件中创建纹理的函数。在计算机图形学和游戏开发中，纹理是用于表现物体外观的图像或图案
        if "asset" in self.cfg["env"]:
    ...
    def compute_full_state(self, asymm_obs=False):
        ...
        计算所有环境的观测值。观测值由三部分组成： 
        左右手的状态值，以及物体和目标的信息。
        每次任务中，左右手的状态值都是相同的，包括手部关节和手指位置、速度和力度信息。
        关节和手指位置、速度和力信息。详细的 422 维 
        观察空间如下图所示：
        ...
        398 - 404	bottle pose  包括位置 (x, y, z) 和方向 (roll, pitch, yaw)。由于瓶子通常是长方形，所以可能会使用一种简化的表示方法，只考虑位置 (x, y, z) 和三个旋转方向,因此是7维。
