...
# class FactoryTaskNutBoltPlaceGPT(FactoryEnvNutBolt, FactoryABCTask):
...
#         ppo_path = 'train/FactoryTaskNutBoltPlaceGPTPPO.yaml'  # relative to Gym's Hydra search path (cfg dir)
...
#     def compute_reward(self):
        # self.rew_buf[:], self.rew_dict = quaternion_product(self.quat1, self.quat2)
...
class FactoryTaskNutBoltPlaceGPT(FactoryEnvNutBolt, FactoryABCTask):
    ...
    def _acquire_task_tensors(self):  #diferentence
        ...
        self.bolt_tip_pos=torch.zeros_like(self.bolt_pos, device=self.device) #太高了
...
from torch import Tensor
@torch.jit.script
def quaternion_product(quat1: Tensor, quat2: Tensor) -> Tensor:
    """Calculate the product of two quaternion tensors."""
    result = torch.zeros_like(quat1)
    result[..., 0] = quat1[..., 0] * quat2[..., 0] - torch.sum(quat1[..., 1:] * quat2[..., 1:], dim=-1)
    result[..., 1:] = quat1[..., 0:1] * quat2[..., 1:] + quat2[..., 0:1] * quat1[..., 1:] + torch.cross(quat1[..., 1:], quat2[..., 1:])
    return result
