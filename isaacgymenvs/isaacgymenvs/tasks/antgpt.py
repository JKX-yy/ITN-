...
class AntGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.obs_buf, self.potentials, self.prev_potentials)
...
@torch.jit.script
def compute_reward(obs_buf: torch.Tensor, potentials: torch.Tensor, prev_potentials: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Constants for reward calculations
    speed_scale: float = 0.01  # Adjusted scale for speed_reward
    potential_gain_scale: float = 1.0  # New scale for potential_gain_reward
    speed_temp: float = 10.0  # Temperature parameter for speed_reward
    potential_gain_temp: float = 10.0  # Temperature parameter for potential_gain_reward

    # Extracting necessary components from obs_buf
    forward_velocity = obs_buf[:, 1]  # Assuming second column is forward velocity

    # Calculating potential difference more robustly to avoid inf values
    potential_difference = torch.clamp(potentials - prev_potentials, min=-10.0, max=10.0)  # Clamping to avoid extreme values

    # Reward calculations
    speed_reward = torch.exp((forward_velocity * speed_scale) / speed_temp) - 1.0  # Normalizing and scaling
    potential_gain_reward = torch.exp((potential_difference * potential_gain_scale) / potential_gain_temp) - 1.0  # Normalizing and scaling

    # Total reward combines both components
    total_reward = speed_reward + potential_gain_reward

    # Reward components dictionary for analysis
    reward_components = {
        "speed_reward": speed_reward,
        "potential_gain_reward": potential_gain_reward
    }
