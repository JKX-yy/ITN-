...
def compute_reward(
    ...
    rotation_error_temp = 1.0
    angvel_temp = 0.1

    # Compute quaternion distance between object_rot and goal_rot
    rotation_distance = 1 - (torch.sum(object_rot * goal_rot, dim=1) ** 2)
    
    # Calculate the angular velocity error
    angvel_error = torch.norm(object_angvel, dim=1)
    
    # Transform the rotation_distance and angvel_error using torch.exp
    rotation_error_reward = -torch.exp(rotation_error_temp * rotation_distance) + 1
    angvel_reward = -torch.exp(angvel_temp * angvel_error) + 1

    # Combine the rewards
    total_reward = rotation_error_reward + angvel_reward
    
    reward_components = {
        "rotation_error_reward": rotation_error_reward,
