...
ISAAC_ROOT_DIR = f"{ITN_ROOT_DIR}/../isaacgymenvs/isaacgymenvs"

@hydra.main(config_path="cfg", config_name="config", version_base="1.1")
def main(cfg):
    workspace_dir = Path.cwd()
    logging.info(f"Workspace: {workspace_dir}")
    logging.info(f"Project Root: {ITN_ROOT_DIR}")

    openai.api_key = cfg.gpt_key
    openai.api_base=cfg.gpt_url 
    ...
    env_parent = 'isaac' if f'{env_name}.py' in os.listdir(f'{ITN_ROOT_DIR}/envs/isaac') else 'dexterity'
    task_file = f'{ITN_ROOT_DIR}/envs/{env_parent}/{env_name}.py'
    task_obs_file = f'{ITN_ROOT_DIR}/envs/{env_parent}/{env_name}_obs.py'
    ...
    prompt_dir = f'{ITN_ROOT_DIR}/utils/prompts'
    ...
    for iter in range(cfg.iteration):  
        #Iteration 1: Generating 8 samples with gpt-3.5-turbo-16k-0613
        # Get ITN response
        responses = []
        response_cur = None
        total_samples = 0
        ...
        chunk_size = cfg.sample  if "gpt-3.5" in model else 1
        ...
        #Iteration 0: Prompt Tokens: 622, Completion Tokens: 3576, Total Tokens: 4198
        code_runs = [] 
        rl_runs = []
        for response_id in range(cfg.sample):  
            ...
            # Add the ITN Reward Signature to the environment code 
            try:
                gpt_reward_signature, input_lst = get_function_signature(code_string)   #code_string  
            except Exception as e:
        ...
        for response_id, (code_run, rl_run) in enumerate(zip(code_runs, rl_runs)):  
            ...
            content = ''
            traceback_msg = filter_traceback(stdout_str)    # Return an empty string if no Traceback is found
            ...
            if traceback_msg == '':
                ...
                for i, line in enumerate(lines):
                    if line.startswith('Tensorboard Directory:'): #ine.startswith(‘Tensorboard Directory:’) 
                        break 
                tensorboard_logdir = line.split(':')[-1].strip()  
                tensorboard_logs = load_tensorboard_logs(tensorboard_logdir)
                max_iterations = np.array(tensorboard_logs['gpt_reward']).shape[0]  #   gt-reward ->gpt_reward
                epoch_freq = max(int(max_iterations // 10), 1)  #10个
    ...
    for i, rl_run in enumerate(eval_runs): 
        ...
        if "gt_reward" in tensorboard_logs and "gpt_reward" in tensorboard_logs :
            ...
            # gt_reward = np.array(tensorboard_logs["gt_reward"])
            # gpt_reward = np.array(tensorboard_logs["gpt_reward"])
            # if gt_reward.shape==gpt_reward.shape:
            #     reward_correlation = np.corrcoef(gt_reward, gpt_reward)[0, 1]
            #     reward_code_correlations_final.append(reward_correlation)
