...
class ShadowHandKettleGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.object_pos, self.kettle_handle_pos, self.bucket_handle_pos, self.right_hand_pos, self.left_hand_pos, self.fingertip_pos, self.fingertip_another_pos)
...
def compute_reward(
    ...
    kettle_handle_pos: torch.Tensor,
    ...
    object_distance_weight = 1.0
    hands_distance_weight = 1.0
    hand_collision_reward_weight = 0.5
    bucket_reward_weight = 1.0

    # Calculate distances
    object_distance = torch.norm(object_pos - kettle_handle_pos)
    distance_between_hands = torch.norm(right_hand_pos - left_hand_pos)

    # Calculate object distance reward
    object_distance_reward = torch.exp(-object_distance)
    reward += object_distance_weight * object_distance_reward
    rewards['object_distance_reward'] = object_distance_reward

    # Calculate hands distance reward
    hands_distance_reward = torch.exp(-distance_between_hands)
    reward += hands_distance_weight * hands_distance_reward
    rewards['hands_distance_reward'] = hands_distance_reward

    # Calculate hand collision reward
    fingertip_distance = torch.min(
        torch.norm(fingertip_pos - RIGHT_HAND_POSITION),
        torch.norm(fingertip_pos - LEFT_HAND_POSITION)
    )
    fingertip_another_distance = torch.min(
        torch.norm(fingertip_another_pos - RIGHT_HAND_POSITION),
        torch.norm(fingertip_another_pos - LEFT_HAND_POSITION)
    )
    hand_collision_reward = torch.exp(-fingertip_distance) + torch.exp(-fingertip_another_distance)
    reward += hand_collision_reward_weight * hand_collision_reward
    rewards['hand_collision_reward'] = hand_collision_reward

    # Calculate bucket reward
    bucket_distance = torch.norm(bucket_handle_pos - kettle_handle_pos)
    bucket_reward = 1.0 if bucket_distance < BUCKET_DISTANCE_THRESHOLD else 0.0
    reward += bucket_reward_weight * bucket_reward
    rewards['bucket_reward'] = bucket_reward
