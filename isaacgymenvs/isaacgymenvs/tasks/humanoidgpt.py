...
class HumanoidGPT(VecTask):
    ...
    def compute_reward(self, actions):
        self.rew_buf[:], self.rew_dict = compute_reward(self.torso_velocity, self.joint_actions)
...
@torch.jit.script
def compute_reward(torso_velocity: torch.Tensor, joint_actions: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
    # Define reward components
    velocity_reward = torso_velocity[:, 0]  # Use only forward velocity

    # Define penalty component
    action_penalty = -torch.norm(joint_actions, p=2, dim=-1)

    # Combine individual rewards with weights
