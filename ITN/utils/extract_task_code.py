...
def get_function_signature(code_string):
    # Parse the code string into an AST 将代码字符串解析为 AST
    module = ast.parse(code_string)

    # Find the function definitions  查找函数定义
    function_defs = [node for node in module.body if isinstance(node, ast.FunctionDef)]

    # If there are no function definitions, return None   如果没有函数定义，则返回 None
    if not function_defs:
        return None

    # For simplicity, we'll just return the signature of the first function definition  为简单起见，我们只返回第一个函数定义的签名
    function_def = function_defs[0]

    input_lst = []
    # Construct the function signature (within object class) 构建函数签名（在对象类内）
